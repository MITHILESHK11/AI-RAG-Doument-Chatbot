...
def extract_images_pdf(file_bytes: bytes, doc_id: str) -> List[Dict]:
    ...
    try:
        ...
        pass
    return imgs
# ----- Diagram / Chart Extraction (extends image extraction) -----
def classify_image_description(caption: str) -> str:
    """
    Heuristic-based classifier for diagrams/charts.
    """
    c = caption.lower()
    if any(x in c for x in ["decision tree", "flowchart", "architecture", "model", "diagram", "block", "process"]):
        return "Diagram"
    elif any(x in c for x in ["graph", "plot", "chart", "bar", "line", "scatter", "histogram"]):
        return "Chart"
...
def extract_diagrams_pdf(file_bytes: bytes, doc_id: str) -> List[Dict]:
    """
    Extract diagrams/charts separately with classification tags.
    """
    diagrams = []
    try:
        doc = fitz.open(stream=file_bytes, filetype="pdf")
        for page_i in range(len(doc)):
            ...
            for img_index, img in enumerate(image_list):
                ...
                try:
                    ...
                    caption = pytesseract.image_to_string(pre, lang='eng').strip()
                except Exception:
                    caption = ""
...
def extract_examples_from_text(pages: List[str], doc_id: str) -> List[Dict]:
    """
    Finds sections labeled as 'Example', 'Case Study', 'Dataset', etc.
    """
    examples = []
    example_pattern = re.compile(r'(Example\s*\d*[:.\-]?\s*|Case\s*Study\s*\d*[:.\-]?\s*|Dataset\s*\d*[:.\-]?)', re.IGNORECASE)
